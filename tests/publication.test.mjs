...
test("requires expiration to be after publication", () => {
  assert.ok(validatePublication(publication({
    expires_at: "2026-08-16T08:07:00.000Z"
  })).some((error) => error.includes("must be after")));
});

test("publication errors are typed and do not contain content", () => {
  assert.throws(
    () => assertPublication(publication({ stories: [{ headline: "secret", body: "" }] })),
    (error) => error instanceof PublicationValidationError
      && error.message === "Publication payload is invalid"
