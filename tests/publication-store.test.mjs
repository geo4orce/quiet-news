...
test("optional current and index reads return null before first publication", async () => {
  const directory = await mkdtemp(path.join(tmpdir(), "quiet-news-store-"));
  const store = new PublicationStore(directory);
  assert.equal(await store.readCurrent({ optional: true }), null);
  assert.equal(await store.readIndex({ optional: true }), null);
});
