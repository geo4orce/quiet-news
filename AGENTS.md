...
- QNP's manual recovery supports `--images-only` for explicitly authorized image completion without sift, or `--with-images` after `--retry-failed-sift`. Both preserve the normal per-image claim and push safeguards.
