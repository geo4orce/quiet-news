...
export function createOpenAIGenerator({
  ...
  async function callResponse(body) {
    ...
    try {
      ...
      throw new GenerationError("malformed_output", false, { requestId, httpStatus: response.status });
    }
    const metadata = responseMetadata(value, requestId);
    if (value.status !== "completed") {
      throw new GenerationError("incomplete_output", false, { ...metadata, httpStatus: response.status });
    }
    try {
      const text = outputText(value, requestId);
      if (text === null) throw new Error("missing output text");
      return { output: JSON.parse(text), metadata, httpStatus: response.status };
    } catch (error) {
      if (error instanceof GenerationError) {
        error.metadata.httpStatus = response.status;
        throw error;
      }
      throw new GenerationError("malformed_output", false, { ...metadata, httpStatus: response.status });
    }
  }
  ...
  async function runStage({ stage, targetDate, promptVersion, body, validate, describe, priorProviderAttempts = 0 }) {
    ...
    for (let attempt = 1; ; attempt += 1) {
      ...
      try {
        ...
        error.metadata = {
          ...error.metadata,
          stage, targetDate, attempts: attempt, maxAttempts, timeoutMs,
  ...
  async function generate({ editionDay, priorEdition }) {
    ...
    await archiveStage("discovery", discovery);
