...
import { outputFailureDetails, outputFailureSummary } from "./output-diagnostics.mjs";
...
async function runBackgroundResponse({
  ...
  softTimeoutMs = 300_000, hardTimeoutMs = 600_000, pollMs = 5_000,
  ...
  const stageName = stage === "discovery" ? "Collection" : "Sift";
  ...
  if (!state || state.status === "failed") {
    ...
    logger.info?.(`${stageName} started (attempt ${attempt}).`);
  ...
  for (;;) {
    ...
    if (!warned && age >= softTimeoutMs) {
      warned = true;
      logger.warn?.(`${stageName} still running after ${Math.round(age / 1000)}s; waiting for the same request.`);
    }
    if (!result) {
      try { result = await exchange("GET", `/${state.responseId}`); }
        ...
        if (!pollWarned) logger.warn?.("Response check failed; retrying the same request.");
        pollWarned = true;
        await sleep(pollMs);
        continue;
      }
    ...
    if (value.status === "completed") {
      ...
      try {
        ...
        const code = error?.errorCode === "provider_refusal" ? "provider_refusal" : "malformed_output";
        const details = outputFailureDetails(error, phase);
        const metadata = { ...responseMetadata(value, requestId), httpStatus,
          durationMs: Math.max(0, nowMs() - Date.parse(state.startedAt)) };
        await persist({ ...state, status: "failed", retryable: false, code,
          failedAt: new Date(nowMs()).toISOString(), failure: { ...details, ...metadata } });
        logger.error?.(`${stageName} output rejected.${outputFailureSummary(details)}`);
      ...
      logger.info?.(`${stageName} response validated in ${Math.round(metadata.durationMs / 1000)}s.`);
...
export async function backgroundResponse(options) {
  try { return await runBackgroundResponse(options); }
    ...
    if (error instanceof GenerationError) {
      const code = generationFailureDetails(error).code;
      options.logger?.error?.(`${options.stage === "discovery" ? "Collection" : "Sift"} failed: ${code.replaceAll("_", " ")}.`);
    }
    throw error;
  }
}
